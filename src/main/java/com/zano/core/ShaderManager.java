...
public class ShaderManager {
    ...
    public void createSunLightUniform(String uniformName) throws Exception{
        for(int i = 0; i < Consts.MAX_LIGHTS; i++) {
            createUniform(uniformName + "[" + i + "]" + ".colour");
            //createUniform(uniformName + ".pos");
            createUniform(uniformName + "[" + i + "]" + ".intensity");
        }
    }
    ...
    public void setUniform(String uniformName, List<SunLight> sunLights){
        for(int i = 0; i < Consts.MAX_LIGHTS; i++) {
            if(i < sunLights.size()) {
                setUniform(uniformName + "[" + i + "]" + ".colour", sunLights.get(i).getColour());
                //setUniform(uniformName + ".pos", sunLight.getDirection());
                setUniform(uniformName + "[" + i + "]" + ".intensity", sunLights.get(i).getIntensity());
            }
            else{
                setUniform(uniformName + "[" + i + "]" + ".colour", new Vector3f(0,0,0));
                setUniform(uniformName + "[" + i + "]" + ".intensity", new Vector3f(0,0,0));
            }
        }
    }
